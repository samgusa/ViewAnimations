...
//  ToggleTest
//
//  Created by Sam Greenhill on 5/13/23.
//
...
struct CustomToggleStyle: ToggleStyle {
    ...
    // the sf symbol to use for the toggle's off state
        ...
        HStack {
            RoundedRectangle(cornerRadius: 60)
                ...
                .overlay {
                    ...
                    // the on and off points for the circle. when the toggle is tapped, the circle will be moved btw these two points.
                    .offset(x: configuration.isOn ? 50 : -50)
                    .offset(x: offset.width, y: offset.height)
                    .gesture(
                        DragGesture()
                        // setup dragging the circle. if the circle is within the stickiness distance from where it started when the drag distance is manipulated to be exponetially shorter. This adds a feeling like the circle doesn't want to leave the toggle. If the circle gets past the stickiness distance then it snaps to the gesture's current position. Visually this means the circle jumps to the user's finger.
                        ...
                        // when the user lifts the finger snap the circle back to the toggle with some springiness. this also toggles the toggle state
                            .onEnded { _ in
