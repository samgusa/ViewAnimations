...
struct FidgetTextView: View {
    ...
    private var unselectedScale: CGFloat = 1
    private var selectedScale: CGFloat = 1.7
    private var adjacentScale: CGFloat = 1.2
...
extension FidgetTextView {
            ...
            .onChanged { value in
                ...
                if let index = self.calculateSelectedIndex(for: dragPosition) {
                    if index != selectedIndex {
                        ...
                        HapticsManager.shared.generateFeedback()
    ...
    private func calculateSelectedIndex(for position: CGPoint) -> Int? {
        //calculate distance btw position and center of each character
        let distances = characterCenters.map { distance($0, position) }

        //get index of te character closest to the position
        guard let closestIndex = distances.enumerated().min(by: { $0.1 < $1.1 })?.offset else {
            return nil
        }
        return closestIndex
    }
